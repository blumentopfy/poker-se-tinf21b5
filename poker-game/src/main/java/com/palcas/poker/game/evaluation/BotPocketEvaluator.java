...
public class BotPocketEvaluator {
    ...
    /*
    creates a given number of random virtual pockets and community cards to form combinations.
    then evaluates against how many virtual pockets the original pocket would win or draw against.
    @param pocketCards original pocket cards of the bot
    @param numberPockets number of scenarios the pocketCards shall be evaluated in
    ...
    public int evaluatePreFlopPocket(ArrayList<Card> pocketCards, int numberPockets) {
        ...
        for(int scenarioCounter = 0; scenarioCounter < numberPockets; scenarioCounter++) {
            ...
            // this Array contains the community cards at index 0-4 and the simulated pockets at index 5-6
            Card[] communityCardsWithSimulatedPocket = new Card[7];
            for (int i = 0; i < 7; i++) {
                communityCardsWithSimulatedPocket[i] = deck.get(i);
            }

            // this Array contains the community cards at index 0-4 (they are copied from other array) and the 2 original pocket cards
            ...
            if(handEvaluationService.compare(communityCardsWithOriginalPocket, communityCardsWithSimulatedPocket) >= 1) {
                counterTimesWonOrEqual++;
            }
        }
        return counterTimesWonOrEqual;
